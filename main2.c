...
float calcularVolumen(float puntos [5][3], float area1, float baricentro);
...
int main(int argc, char const *argv[])
{
    ...
    float volumen = calcularVolumen(puntos,area1,baricentro);
...
float calcularVolumen(float puntos [5][3], float area1, float baricentro){
    float volumen = ((1/3)*(area1)*(baricentro));
    return volumen;
}